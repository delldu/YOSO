...
class FFN(nn.Module):
    def __init__(self, embed_dims=256, feedforward_channels=1024, num_fcs=2):
        ...
        for _ in range(num_fcs - 1):
            layers.append(nn.Sequential(nn.Linear(in_channels, feedforward_channels), nn.ReLU(True), nn.Dropout(0.0)))
...
class DySepConvAtten(nn.Module):
    ...
    def forward(self, query, value):
        ...
        dy_point_conv_weight = dy_conv_weight[:, :, self.kernel_size :].view(
            B, self.num_proposals, self.num_proposals, 1
        )
...
class CrossAttenHead(nn.Module):
    def __init__(self):
        ...
        self.s_atten = nn.MultiheadAttention(
            embed_dim=self.hidden_dim * self.conv_kernel_size_2d**2, num_heads=8, dropout=0.0
        )
    ...
    def forward(
        self, features, proposal_kernels, mask_preds, train_flag: bool
        ...
        return (
            ...
            obj_feat.permute(0, 1, 3, 2).reshape(
                B, self.num_proposals, self.hidden_dim, self.conv_kernel_size_2d, self.conv_kernel_size_2d
            ),
        )
